...
import styles from './PaginationSuspensionTable.module.css' //./PaginationTable.module.css'
...
export type PaginationTableProps = {
	...
	reload: boolean
	setReload: Function
    //filtroToFilter: string,
	tipoBusquedaIdToFilter: number,
	fetchDataFunction: (
		filter: ObjFiltro,
		pageNumber: number,
		size: number,
...
export default function PaginationSuspencionTable(props: PaginationTableProps) {

	const [currentData, setCurrentData] = useState<any>([])
	const [activePage, setActivePage] = useState(1)
	const [size, setSize] = useState(10)
	const [totalPages, setTotalPages] = useState(1)
    const [filtroToFilter, setFiltroToFilter] = useState<string>( )
	const [tipoBusquedaIdToFilter] = useState(props.tipoBusquedaIdToFilter)
	...
	useEffect(() => {
		...
		if (tokenData != null && filtroToFilter != null && filtroToFilter != '') {
			let tokenObj: TokenResponseDTO = JSON.parse(tokenData)
			console.log(tokenObj)

			let objFiltro: ObjFiltro = {
                tipoBusquedaId: tipoBusquedaIdToFilter,
                filtro: filtroToFilter
			}
			fetchData(objFiltro, activePage, size, handleResponse, handleError, searchSortItem?.search, searchSortItem?.sort)
		}

	}, [tokenData, filtroToFilter, tipoBusquedaIdToFilter, fetchData, reload, setReload, activePage, size, searchSortItem])
		...
		return (
			<div className={styles.header} >
				<Form>
					<Form.Group>
                        <Form.Input
							label="Nombre del propietario o representante legal"
							defaultValue={filtroToFilter}
							onBlur={(e:any) => setFiltroToFilter(e.target.value)}
						/>
					</Form.Group>
				</Form>
...
PaginationSuspencionTable.defaultProps = {
	noAddButton: false,
	addButtonIcon: 'add',
